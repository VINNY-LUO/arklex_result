...
def conversation(model_api, goal, summary, model_params, synthetic_data_params, env_config):
    ...
    for i in range(synthetic_data_params['max_turns']):
        ...
        try:
            nlu_records = response_data['parameters']['taskgraph']['nlu_records']
            pred_intent = nlu_records[-1]['pred_intent'] if nlu_records else None
        except (KeyError, IndexError) as e:
            print(f"Warning: Could not get pred_intent - {str(e)}")
            pred_intent = None
    ...
    trajectory = model_params.get("memory", {}).get("history", [])  
    history.append({'trajectory': trajectory})
    return history
